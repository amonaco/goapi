/* tslint:disable */
// api v0.0.1 b799fcfe5888ca55d4bff83e189ea56a360da861
// --
// This file has been generated by https://github.com/webrpc/webrpc using gen/typescript
// Do not edit by hand. Update your webrpc schema and re-generate.

// WebRPC description and code-gen version
export const WebRPCVersion = "v1"

// Schema version of your RIDL schema
export const WebRPCSchemaVersion = "v0.0.1"

// Schema hash generated from your RIDL schema
export const WebRPCSchemaHash = "b799fcfe5888ca55d4bff83e189ea56a360da861"


//
// Types
//
export interface Status {
  success: boolean
  error: string
  id: number
  message: string
}

export interface Permission {
  id?: number
  document_id: number
  user_id: number
  permission: {[key: string]: any}
}

export interface PermissionService {
  create(args: CreateArgs, headers?: object): Promise<CreateReturn>
  get(args: GetArgs, headers?: object): Promise<GetReturn>
  getByUser(args: GetByUserArgs, headers?: object): Promise<GetByUserReturn>
  check(args: CheckArgs, headers?: object): Promise<CheckReturn>
  update(args: UpdateArgs, headers?: object): Promise<UpdateReturn>
  remove(args: RemoveArgs, headers?: object): Promise<RemoveReturn>
}

export interface CreateArgs {
  permission: Permission
}

export interface CreateReturn {
  status: Status  
}
export interface GetArgs {
  id: number
}

export interface GetReturn {
  result: Permission  
}
export interface GetByUserArgs {
  id: number
}

export interface GetByUserReturn {
  result: Array<Permission>  
}
export interface CheckArgs {
  id: number
}

export interface CheckReturn {
  result: Status  
}
export interface UpdateArgs {
  permission: Permission
}

export interface UpdateReturn {
  status: Status  
}
export interface RemoveArgs {
  id: number
}

export interface RemoveReturn {
  status: Status  
}


  
//
// Client
//
export class PermissionService implements PermissionService {
  private hostname: string
  private fetch: Fetch
  private path = '/rpc/PermissionService/'

  constructor(hostname: string, fetch: Fetch) {
    this.hostname = hostname
    this.fetch = fetch
  }

  private url(name: string): string {
    return this.hostname + this.path + name
  }
  
  create = (args: CreateArgs, headers?: object): Promise<CreateReturn> => {
    return this.fetch(
      this.url('Create'),
      createHTTPRequest(args, headers)).then((res) => {
      return buildResponse(res).then(_data => {
        return {
          status: <Status>(_data.status)
        }
      })
    })
  }
  
  get = (args: GetArgs, headers?: object): Promise<GetReturn> => {
    return this.fetch(
      this.url('Get'),
      createHTTPRequest(args, headers)).then((res) => {
      return buildResponse(res).then(_data => {
        return {
          result: <Permission>(_data.result)
        }
      })
    })
  }
  
  getByUser = (args: GetByUserArgs, headers?: object): Promise<GetByUserReturn> => {
    return this.fetch(
      this.url('GetByUser'),
      createHTTPRequest(args, headers)).then((res) => {
      return buildResponse(res).then(_data => {
        return {
          result: <Array<Permission>>(_data.result)
        }
      })
    })
  }
  
  check = (args: CheckArgs, headers?: object): Promise<CheckReturn> => {
    return this.fetch(
      this.url('Check'),
      createHTTPRequest(args, headers)).then((res) => {
      return buildResponse(res).then(_data => {
        return {
          result: <Status>(_data.result)
        }
      })
    })
  }
  
  update = (args: UpdateArgs, headers?: object): Promise<UpdateReturn> => {
    return this.fetch(
      this.url('Update'),
      createHTTPRequest(args, headers)).then((res) => {
      return buildResponse(res).then(_data => {
        return {
          status: <Status>(_data.status)
        }
      })
    })
  }
  
  remove = (args: RemoveArgs, headers?: object): Promise<RemoveReturn> => {
    return this.fetch(
      this.url('Remove'),
      createHTTPRequest(args, headers)).then((res) => {
      return buildResponse(res).then(_data => {
        return {
          status: <Status>(_data.status)
        }
      })
    })
  }
  
}

  
export interface WebRPCError extends Error {
  code: string
  msg: string
	status: number
}

const createHTTPRequest = (body: object = {}, headers: object = {}): object => {
  return {
    method: 'POST',
    headers: { ...headers, 'Content-Type': 'application/json' },
    body: JSON.stringify(body || {})
  }
}

const buildResponse = (res: Response): Promise<any> => {
  return res.text().then(text => {
    let data
    try {
      data = JSON.parse(text)
    } catch(err) {
      throw { code: 'unknown', msg: `expecting JSON, got: ${text}`, status: res.status } as WebRPCError
    }
    if (!res.ok) {
      throw data // webrpc error response
    }
    return data
  })
}

export type Fetch = (input: RequestInfo, init?: RequestInit) => Promise<Response>
