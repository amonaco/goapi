...
func WebRPCSchemaHash() string {
	return "ca5eebb31b5c36c2b9324455ea29be5d484f89dc"
}
...
type AuthService interface {
	...
	Logout(ctx context.Context, authToken string) (*Status, error)
	SetPassword(ctx context.Context, creds *PasswordToken) (*Status, error)
...
func (s *authServiceServer) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	case "/rpc/AuthService/Logout":
		...
		return
	case "/rpc/AuthService/SetPassword":
		s.serveSetPassword(ctx, w, r)
		return
	case "/rpc/AuthService/ForgotPassword":
		s.serveForgotPassword(ctx, w, r)
		return
	default:
...
func (s *authServiceServer) serveLoginJSON(ctx context.Context, w http.ResponseWriter, r *http.Request) {
	...
	func() {
		...
		ret0, err = s.AuthService.Login(ctx, reqContent.Arg0)
	}()
	respContent := struct {
		Ret0 string `json:"authToken"`
	}{ret0}
...
func (s *authServiceServer) serveLogoutJSON(ctx context.Context, w http.ResponseWriter, r *http.Request) {
	...
	func() {
		...
		ret0, err = s.AuthService.Logout(ctx, reqContent.Arg0)
	}()
	respContent := struct {
...
func (s *authServiceServer) serveSetPassword(ctx context.Context, w http.ResponseWriter, r *http.Request) {
	header := r.Header.Get("Content-Type")
	i := strings.Index(header, ";")
	if i == -1 {
		i = len(header)
	...
	switch strings.TrimSpace(strings.ToLower(header[:i])) {
	case "application/json":
		s.serveSetPasswordJSON(ctx, w, r)
	default:
		err := Errorf(ErrBadRoute, "unexpected Content-Type: %q", r.Header.Get("Content-Type"))
		RespondWithError(w, err)
	}
...
func (s *authServiceServer) serveSetPasswordJSON(ctx context.Context, w http.ResponseWriter, r *http.Request) {
	var err error
	ctx = context.WithValue(ctx, MethodNameCtxKey, "SetPassword")
	reqContent := struct {
		Arg0 *PasswordToken `json:"creds"`
	}{}
	...
	func() {
		...
		ret0, err = s.AuthService.SetPassword(ctx, reqContent.Arg0)
	}()
	respContent := struct {
		Ret0 *Status `json:"status"`
	}{ret0}
...
func (s *authServiceServer) serveForgotPassword(ctx context.Context, w http.ResponseWriter, r *http.Request) {
	header := r.Header.Get("Content-Type")
	i := strings.Index(header, ";")
	if i == -1 {
		i = len(header)
	...
	switch strings.TrimSpace(strings.ToLower(header[:i])) {
	case "application/json":
		s.serveForgotPasswordJSON(ctx, w, r)
	default:
		err := Errorf(ErrBadRoute, "unexpected Content-Type: %q", r.Header.Get("Content-Type"))
		RespondWithError(w, err)
	}
...
func (s *authServiceServer) serveForgotPasswordJSON(ctx context.Context, w http.ResponseWriter, r *http.Request) {
	var err error
	ctx = context.WithValue(ctx, MethodNameCtxKey, "ForgotPassword")
	reqContent := struct {
	...
	func() {
		...
		err = s.AuthService.ForgotPassword(ctx, reqContent.Arg0)
	}()
